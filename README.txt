...
Client:
The client reads the configuration file, updates the sensor packets accordingly (with current, min, and max temperature information, as well as the action status), and then sends to the server after serializing the packet into a string.  The serialization code is found in the sensor.c file.  The client will then send an action packet full of 0s and with an action set to 1 (extra credit). When the action is set to 1 it will then wait to receive a status message back.  The status will be printed to the terminal regarding the overtemp (if one, two, or no sensors are over temperature).  All of the networking code is taken care of in the eztcp.c file; all of the network calls reference this library.  
